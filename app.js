...
if (!resultados || resultados.querySelector(’.empty-state’)) {
alert(currentLanguage === ‘es’
? ‘⚠️ Primero analiza una inversión para poder exportar el PDF.’
: ‘⚠️ Please analyze an investment first before exporting the PDF.’);
return;
}
...
// Estado de carga
btn.disabled = true;
btn.innerHTML = '<span class="pdf-spinner"></span> ' +
    (currentLanguage === 'es' ? 'Generando PDF...' : 'Generating PDF...');

try {
    // Esperar a que las librerías estén disponibles
    if (typeof window.jspdf === 'undefined' || typeof html2canvas === 'undefined') {
        throw new Error('Las librerías PDF no están disponibles. Recarga la página e inténtalo de nuevo.');
    }

    const { jsPDF } = window.jspdf;

    // Ocultar elementos que no queremos en el PDF
    const floatBar = document.getElementById('floatingSummary');
    const cookieBanner = document.getElementById('cookie-banner');
    const originalFloatDisplay = floatBar ? floatBar.style.display : '';
    const originalCookieDisplay = cookieBanner ? cookieBanner.style.display : '';
    if (floatBar) floatBar.style.display = 'none';
    if (cookieBanner) cookieBanner.style.display = 'none';

    // Capturar el panel de resultados
    const resultsPanel = document.querySelector('.results-panel');
    const targetEl = resultsPanel || document.querySelector('.main-content') || document.body;
    ...
    const canvas = await html2canvas(targetEl, {
        ...
        windowWidth: Math.max(1024, targetEl.scrollWidth),
        scrollX: 0,
        scrollY: 0,
        allowTaint: true
    });
    ...
    if (floatBar) floatBar.style.display = originalFloatDisplay;
    if (cookieBanner) cookieBanner.style.display = originalCookieDisplay;
    ...
    pdf.text(currentLanguage === 'es'
        ? '🏠 Análisis de Inversión Inmobiliaria'
        : '🏠 Real Estate Investment Analysis', margin, 14);
    ...
    pdf.text(`pharmajava.github.io  ·  ${fecha}`, pageW - margin, 14, { align: 'right' });

    // Convertir canvas a imagen y paginar
    const imgData = canvas.toDataURL('image/jpeg', 0.92);
    const imgWidthPx = canvas.width;
    const imgHeightPx = canvas.height;
    const imgDisplayW = contentW;
    const imgDisplayH = (imgHeightPx * imgDisplayW) / imgWidthPx;

    const startY = 26; // debajo del header
    ...
    while (renderedPx < imgHeightPx) {
        const slicePx = Math.round(availableH * imgWidthPx / imgDisplayW);
        const actualSlicePx = Math.min(slicePx, imgHeightPx - renderedPx);
        const sliceDisplayH = (actualSlicePx * imgDisplayW) / imgWidthPx;
        ...
        ctx.drawImage(canvas, 0, renderedPx, imgWidthPx, actualSlicePx, 0, 0, imgWidthPx, actualSlicePx);
        ...
        if (!isFirstPage) {
            ...
            pdf.setFont('helvetica', 'bold');
            pdf.text(currentLanguage === 'es'
                ? 'Análisis de Inversión Inmobiliaria (cont.)'
                : 'Real Estate Investment Analysis (cont.)', margin, 14);
        }

        pdf.addImage(sliceData, 'JPEG', margin, isFirstPage ? startY : 26, imgDisplayW, sliceDisplayH);

        renderedPx += actualSlicePx;
        isFirstPage = false;
    }
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        pdf.text(
            currentLanguage === 'es'
                ...
                : 'Indicative tool — does not constitute financial advice',
            margin, pageH - 5
        );
    ...
    const precio = precioEl ? precioEl.value : '0';
    const fechaFile = new Date().toLocaleDateString('es-ES').replace(/\//g, '-');
    pdf.save(`analisis-inversion-${precio}eur-${fechaFile}.pdf`);
...
if (precioInput) {
    precioInput.addEventListener('input', () => { actualizarEntradaSlider(); calcular(); });
}
if (entradaEurosInput) {
    entradaEurosInput.addEventListener('input', () => { actualizarEntradaSlider(); calcular(); });
}
if (entradaSlider) {
...
// Exponer actualizarITPHint para resetCalculadora
window.actualizarITPHint = actualizarITPHint;

if (ccaaSelector) {
    ccaaSelector.addEventListener('change', () => { actualizarITPHint(); calcular(); });
}

if (tipoViviendaSel) {
    tipoViviendaSel.addEventListener('change', () => { actualizarITPHint(); calcular(); });
}

actualizarITPHint();

const resetBtn = document.getElementById('resetBtn');
if (resetBtn) resetBtn.addEventListener('click', resetCalculadora);

const calcularBtn = document.getElementById('calcularBtn');
if (calcularBtn) calcularBtn.addEventListener('click', calcular);

const shareBtn = document.getElementById('shareBtn');
if (shareBtn) shareBtn.addEventListener('click', compartirAnalisis);
